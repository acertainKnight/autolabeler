...
class CorporateOpenAIClient(ChatOpenAI):
    ...
    def with_structured_output(self, schema: type[T], **kwargs: Any) -> Any:
        ...
        Args:
            ...
            **kwargs: Additional arguments passed to the underlying method. Common kwargs include:
                - method: The method to use for structured output. Options include:
                    * "function_calling": Uses tool/function calling (recommended for OpenRouter/OpenAI-compatible APIs)
                    * "json_schema": Uses OpenAI's structured output API (may not work with all providers)
                    * "json_mode": Uses JSON mode with manual parsing
        ...
        Example:
            ...
            ...     name: str
            ...     age: int
            >>> # For OpenRouter/OpenAI-compatible APIs, use method="function_calling"
            >>> structured_client = client.with_structured_output(Person, method="function_calling")
            >>> person = structured_client.invoke("Alice is 30 years old")
            >>> assert isinstance(person, Person)
        """
        try:
            # For compatibility with various OpenAI-compatible providers, default to function_calling
            if "method" not in kwargs and hasattr(super(), "with_structured_output"):
                logger.info(f"Using method='function_calling' for structured output with schema {schema.__name__}")
